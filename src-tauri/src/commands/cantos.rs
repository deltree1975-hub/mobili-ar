...
#[tauri::command]
pub fn crear_canto(state: State<DbState>, datos: CrearCantoInput) -> Result<Canto, String> {
    let guard = state.0.lock().map_err(|e| e.to_string())?;
    let conn = guard.as_ref().ok_or("Base de datos no conectada")?;
    crate::db::cantos::crear(conn, &datos).map_err(|e| e.to_string())
}

#[tauri::command]
pub fn actualizar_canto(state: State<DbState>, id: String, datos: CrearCantoInput) -> Result<(), String> {
    let guard = state.0.lock().map_err(|e| e.to_string())?;
    let conn = guard.as_ref().ok_or("Base de datos no conectada")?;
    crate::db::cantos::actualizar(conn, &id, &datos).map_err(|e| e.to_string())
}
